...
def WS(x, y, radius, clearance):
    ...
    obstacle2 = (x >= 1.5 - total_space) and (x <= 1.625 + total_space) and (y >= 0.75 - total_space)
    obstacle3 = (x >= 2.5 - total_space) and (x <= 2.625 + total_space) and (y <= 1.25 + total_space)

    # Check for the Circular obstacle 
    obstacle1 = ((np.square(x - 4)) + (np.square(y - 1.1)) <= np.square(0.5 + total_space))
